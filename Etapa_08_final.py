...
def seleccion_palabras_para_el_rosco(diccionario, CANTIDAD_LETRAS_ROSCO):
    ...
    return cant_letras_al_azar, diccionario_resultado

def Convertir_Dicc_Palabras(diccionario_final):
    """
...
def mostrar_diccionario_candidato(long_pal_min,cant_letras_rosco):
    """
    ...
    diccionario_seleccion = palabras_segun_longitud(diccionario, long_pal_min) #LONGITUD_PALABRA_MINIMA
    letras_seleccionadas,diccionario_final = seleccion_palabras_para_el_rosco(diccionario_seleccion, cant_letras_rosco) #CANT_LETRAS_ROSCO
    lista_final = Convertir_Dicc_Palabras(diccionario_final)


    archivo_abierto = open('diccionario.csv', 'w', encoding= 'UTF-8')
    #archivo_abierto.truncate(0) (Por si el archivo debe resetearse luego de cada ronda)
    for palabra, definicion in diccionario_final.items():
        archivo_abierto.write(f"{palabra}, {definicion}\n")

    archivo_abierto.close()

    return letras_seleccionadas,lista_final